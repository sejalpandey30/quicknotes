...
export default function Home() {
  ...
  async function fetchNotes() {
    ...
    if (!isSupabaseReady) {
      setError("Supabase is not configured. Update .env.local with valid values.");
      setLoading(false);
      return;
    }
  ...
  async function addNote() {
    ...
    const { error } = await supabase.from("notes").insert({ content: newNote.trim() });
      ...
      <div className="mx-auto w-full max-w-2xl rounded-3xl border border-slate-200 bg-white p-8 shadow-xl shadow-slate-200/50">
        <header className="mb-8 text-center">
          ...
          <p className="mt-3 text-slate-600">
            Add, view, and delete notes stored in the Supabase backend.
          </p>
        </header>
        ...
        <section className="space-y-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <input
              ...
              className="w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-slate-900 outline-none transition duration-150 focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
              disabled={loading}
            />
            <button
              type="button"
